...
7. **`budget.maxCostDollars` is a ceiling, not a fixed price, and only applies to `auto` and `max`.** Websets cost expectations do not carry over; bound cost with `maxItems` and `effort`, and set `budget.maxCostDollars` (`$1` to `$100`) when using `auto` or `max`. Fixed efforts reject `budget`.
