#!/usr/bin/env python3
"""Render people.csv into a self-contained interactive HTML viewer.

Usage:  python3 render_viewer.py <input.csv> <output.html> [--title "Search objective"]

Parses the CSV (stdlib only), embeds the rows as JSON into
viewer/people-viewer.template.html, and writes one HTML file that works
offline: sortable/filterable table, search, expandable per-person details,
clickable profile and source-citation links. The CSV stays the source
...
TEMPLATE = Path(__file__).resolve().parent.parent / "viewer" / "people-viewer.template.html"
PLACEHOLDER = "/*__PAYLOAD__*/ null"


def render(csv_path, html_path, title="People"):
...
def main():
    ...
    ap.add_argument("csv_path", help="input CSV (e.g. people.csv)")
    ap.add_argument("html_path", help="output HTML (e.g. people.html)")
    ap.add_argument("--title", default="People", help="page title, e.g. the search objective")
    a = ap.parse_args()
